...
function row(f) {
  const err = f.probeError
    ...
    : "";
  return `<article class="gm-hunt-card" id="${esc(f.id)}-${esc(f.huntedAt || "")}">
    <div class="gm-frame-top">
      <p class="gm-wall">${esc(f.suggestedWall || "unprobed")}</p>
...
try {
  ...
  meta.textContent = "Could not load hunt findings (is the demo server running?).";
}
